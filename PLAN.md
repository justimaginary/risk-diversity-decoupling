...
更新日期：2026-07-23
当前阶段：SIS-1 已通过 → SIS-2 早期安全风险预测与未见 seed 验证

本文件是仓库唯一的当前执行计划。原风险—多样性解耦主张在 R3
没有通过多 seed 门槛，不能继续作为默认正结论。SIS-1 已证明 D2 的安全
结果具有训练实例依赖性；当前只允许执行 SIS-2：判断这种方向反转能否在
step 30 以前预测，并在未查看的训练 seeds 上前瞻复现。
...
SIS-2A 使用 seeds 42–51 和 step 10/20/30/50/75/100。由于 2026-07-23
更换实例时旧数据盘 checkpoint 没有进入模型或数据归档，必须按 SIS-1
完全相同的训练配置重建 checkpoint。重建运行与原 SIS-1 结果分开保存；
只有最终方向复核通过，才允许用它们做早期轨迹分析。
...
Gate A 不通过则不训练新 seeds，转向随机源分解。Gate A 通过后必须先写入
并哈希 `unseen_seed_predictions.json`，再训练 seeds 52–57。
