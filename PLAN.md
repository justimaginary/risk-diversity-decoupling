...
更新日期：2026-07-23
当前阶段：SIS-3 已完成，Gate C1 Stop → instrumentation-equivalence audit 待决策

本文件是仓库唯一的当前执行计划。原风险—多样性解耦主张在 R3
没有通过多 seed 门槛，不能继续作为默认正结论。SIS-1 已证明 D2 的安全
结果具有训练实例依赖性。SIS-2 已表明 step30 单指标预测器不能在未见
训练 seeds 上可靠前瞻复现。SIS-3 将 seed55/57 的方向变化分别定位到
10-step 和 5-step 窗口，但严格仪器等价复现与跨 seed 共同机制信号未通过；
当前不得启动随机源交换或 batch 反事实。
...
**研究问题**：能否使用不晚于 step 30 的低成本信号预测 step100 的
HarmBench 风险差，并在冻结预测器后对未见 seeds 52–57 做前瞻验证。

SIS-2A 使用 seeds 42–51 和 step 10/20/30/50/75/100。2026-07-23
短暂切换到 bjb2 新实例时数据盘为空，但随后切换到 bjb1 原实例并恢复了
完整 SIS-1 数据盘，因此直接使用原训练运行的 checkpoint，不执行重训。
仓库保留重建脚本只作为数据盘再次丢失时的显式应急入口。

冻结设置：

- 保留预先构造且与正式 HarmBench 100 题零重叠的 monitor-30，不在看到
  SIS-1 结果后扩成新的 40 题集合；
- 每 checkpoint 生成 30 prompts × 8 outputs，generation seed 固定；
- seed45 因 KL=0.910 仅作诊断，不进入主预测拟合；
- 主要目标为连续 step100 HarmBench delta，次要目标沿用
  高 `>=+0.05`、低 `<=-0.05`、其余中间；
- 只比较单指标、风险+拒答、风险+拒答+KL 三类简单线性预测器；
- 模型选择和报告必须使用 leave-one-training-seed-out。

**Gate A（回顾性）**：

- 至少一个不晚于 step30 的冻结指标与最终 delta 的 Spearman rho `>=0.60`；
- LOO 方向正确至少 `7/9`；
- seed42/43 都不能被预测为低风险；
- seed44/46/47/48 至少三个不能被预测为高风险；
- 结果不依赖 seed45。

Gate A 不通过则不训练新 seeds，转向随机源分解。Gate A 通过后先冻结并
哈希预测器，再训练 seeds 52–57；完成 step30 monitor 后必须先写入并哈希
`unseen_seed_predictions.json`，才能运行正式终点评估。

**Gate B（前瞻）**：

- 新 seeds 的预测分数与最终风险 Spearman rho `>=0.50`；
- 非中间实例方向至少 `4/5` 正确；
- 不能把所有实例预测为同类；
- Granite 或 StrongREJECT 至少一个支持相同排序；
- 输出质量全部通过。

SIS-2 仍只使用 RTX 4090/4090D 24GB。Gate B 通过前不做跨模型扩展。

> **2026-07-23 最终状态：Gate SIS-2B = Stop。** step30 P1 在九个
> KL 合格旧 seeds 上达到 Spearman 0.75 和 LOO 7/9，Gate A 通过。
> 预测器冻结后，未见 seeds 52–57 的正式 100 × 8 HarmBench 前瞻
> Spearman 仅 0.377；四个实际非 Middle 实例中仅 2 个方向正确。六个
> 新 seed 的 realized KL 均低于 0.75，输出质量全部通过。按协议停止
> 32-sample 与独立 judge 扩展，不允许事后修改 P1。下一步若执行，必须
> 另行预注册 step30–100 轨迹反转机制分析，重点检查 seed55 与 seed57。

### SIS-3：训练后半程安全轨迹反转

SIS-3 使用与 SIS-2 相同的冻结 monitor-30、每题 8 个输出和固定 generation
seed。SIS-3A 先复评旧 checkpoint，确认 seed55 的风险随训练下降、seed57
随训练上升，并非只能由 monitor/终点数据分布差异解释。

SIS-3B 从 step0 重跑 seeds 52/54/55/57，保存
30/35/40/45/50/55/60/70/80/90/100 checkpoint、完整 continuation state、
每 batch 训练量、固定 probe 和 196 个 LoRA 层/模块组诊断。

结果：

- seed57 在 step40–45 从 Middle 进入 High，定位窗口为 5 steps；
- seed55 的主要风险下降发生在 step60–70，并在 step70–80 从 High
  进入 Middle，分类窗口为 10 steps；
- seed57 的风险上升与拒答率 56.25%→15.42% 同步，seed55 的主要下降
  与拒答率 30.42%→61.25% 同步；
- 四个 dense run 的最终类别、schedule hash、正式 KL<0.75 和输出质量
  均通过；
- 结果前提交的执行脚本还要求 `|dense KL - original KL| <= 0.05`，
  seed52=0.0533、seed55=0.0773，严格仪器等价未通过；
- fixed-probe margin、在线梯度和层级更新没有形成跨目标/控制 seed 一致的
  机制信号。

> **2026-07-23 最终状态：Gate SIS-3C1 = Stop。** 反转存在且被成功
> 定位，但尚不能归因于统一训练量、层组或 batch。为避免事后放宽可执行
> gate，不运行 2×2 随机源交换和局部 batch 反事实。若继续，只允许先做
> 同 seed 原始/仪器化训练逐 step checksum 对照，定位 KL 数值分叉；完整
> 报告见
