...
files can be checked with one command before any claim is escalated. Use
--seeds for independent training seeds and an aggregate gate summary.
"""
...
def main() -> None:
    ...
    reeval_dirs: list[Path] = []
    for seed in seeds:
        train_dir = output_dir / f"train_seed{seed}"
        reeval_dir = output_dir / f"reeval_seed{seed}_{args.num_prompts}x{args.reeval_num_samples}"
        reeval_dirs.append(reeval_dir)
        ...
        run_command(summarize_command, capture_path=output_dir / f"reeval_summary_seed{seed}.txt")
        ...
        if args.target_phrase:
            for label in ("step0", "final"):
                ...
                run_command(audit_command, capture_path=output_dir / f"audit_seed{seed}_{label}.txt")

    aggregate_summary_command = [
        sys.executable,
        str(scripts_dir / "summarize_local_gate.py"),
        *[str(path) for path in reeval_dirs],
        "--bootstrap_samples",
        str(args.bootstrap_samples),
        "--bootstrap_seed",
        str(args.generation_seed),
    ]
    run_command(aggregate_summary_command, capture_path=output_dir / "reeval_summary.txt")
