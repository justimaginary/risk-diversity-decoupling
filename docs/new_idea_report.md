...
1. 使用已冻结的 80 个 zero-overlap AdvBench prompts。
2. 在同一 prompt set 上运行 Qwen3-1.7B DPO。
3. baseline 和 final 都保持每 prompt 32 samples。
4. 计算 RDI 和 Risk Entropy。
5. 同时报 absolute diversity、refusal shift、Guardian risk 和 prompt-stratified 结果。
6. 在 Qwen3-4B 上复核关键 stratum。
7. 加入第二个安全分类器，检查 Granite 结论是否稳健。
